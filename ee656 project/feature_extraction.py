...
def extract_all_features(x, as_vector=False):
    """Run all five extractors and return one combined dict (or vector)."""
    feats = {}
...
if __name__ == "__main__":
    ...
    counts = {"td_": 0, "fd_": 0, "mwt_": 0, "dwt_": 0, "wpt_": 0}
    for n in names:
        for p in counts:
            if n.startswith(p):
                counts[p] += 1
    ...
    # show the non-WPT features (the 32 "fast" ones) for a quick sanity check
    for n in names:
        if not n.startswith("wpt_"):
            print(f"  {n:28s} = {dict(zip(names, vec))[n]: .6g}")
